...
class LogLoss(tf.keras.callbacks.Callback):
    """
    Log losses.
    """
    def __init__(self):
    ...
    def on_epoch_end(self,epoch,logs=None):
        loss = logs['loss']
        lossd = logs['model_loss']
        lossm = logs['model_1_loss']
        lossdm = loss-lossd-lossm
        tf.summary.scalar('lossdm',lossdm,step=epoch)

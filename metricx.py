...
def Pvalue(A,B,alt='less',type='paired-t'):
# def Pvalue(A,B,alt='less',type='signed-rank'):
    """
    ...
    a = np.array(A)
    b = np.array(B)
    if type == 'signed-rank': w,p = stats.wilcoxon(a,b,correction=True, alternative=alt)
    if type == 'paired-t':
